document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchActivities() {
    try {
      ...
      // Clear loading message and reset activity select options (keep placeholder)
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsListHTML = details.participants && details.participants.length
          ? details.participants.map(p => `<li><span class="participant-email">${p}</span><button class="participant-delete" data-email="${p}" data-activity="${name}" title="Remove participant">✖</button></li>`).join("")
